...
import { createMiraDocumentLibrary, CARE_DOCUMENT_LIBRARY_PATIENT_ID, CARE_DOCUMENT_LIBRARY_VERSION } from './care-document-library';
import { CareCircle } from './care-circle';
import { createInitialCareCircle, canFamilyOpenView, activeCareCircleMember, type CareCircleState } from './care-circle-state';
import { CareTaskInbox } from './care-task-inbox';
import type { CareTaskAssignee } from './family-care-state';
import { CareMedicines } from './care-medicines';
import { CareConversationRequestForm } from './care-conversation-request';
import type { CareConversationRequest } from './care-conversation-request-state';
import { careConversationRequestEpisodeId, recordCareConversationImpactEvent, type CareConversationImpactEpisode, type CareConversationImpactStage, type CareConversationInitiator } from './care-conversation-impact.ts';
import { CareMedicineAccess } from './care-medicine-access';
import type { MedicineAccessEntry } from './care-medicine-access-state';
import { appendCareNoteAcknowledgement, hasTypedCareNoteDoctorSignature, type CareNoteAcknowledgement } from './care-note-signing-state';
...
import { CareNextVisit } from './care-next-visit';
...
import { addCareStoryEntry, type CareStoryEntry } from './family-care-story-state';
import type { ConfirmedDocumentTreatment } from './care-document-treatment-state';
import type { HandoverContact } from './family-handover-state';
import { INITIAL_FAMILY_TASKS, addFamilyTask, editFamilyTask, removeFamilyTask, restoreFamilyTask, setFamilyTaskCompleted, type FamilyTask } from './family-care-state';
...
export function ContinuityPrototype() {
  ...
  const [librarySeeded, setLibrarySeeded, librarySave] = useCareLocalState<number>('document-library-v1', 0);
  const [careCircle, setCareCircle, circleSave] = useCareLocalState<CareCircleState>('care-circle', createInitialCareCircle());
  ...
  const [conversationRequests, setConversationRequests, requestSave] = useCareLocalState<CareConversationRequest[]>('conversation-requests', []);
  const [activeConversationRequests, setActiveConversationRequests, requestLinkSave] = useCareLocalState<Record<string, string>>('conversation-request-links', {});
  const [conversationImpactEpisodes, setConversationImpactEpisodes, impactSave] = useCareLocalState<CareConversationImpactEpisode[]>('conversation-impact-episodes', []);
  const [activeImpactEpisodes, setActiveImpactEpisodes] = useCareLocalState<Record<string, string>>('conversation-impact-active', {});
  const [medicineAccess, setMedicineAccess, medicineAccessSave] = useCareLocalState<Record<string, MedicineAccessEntry>>('medicine-access', {});
  const [noteSelection, setNoteSelection] = useState<{ patientId: string; version: number } | null>(null);
  ...
  useEffect(() => {
    if ([librarySave, reportSave, documentSave, eventSave, taskSave].includes('loading') || librarySeeded >= CARE_DOCUMENT_LIBRARY_VERSION) return;
    let active = true;
    createMiraDocumentLibrary(CARE_DOCUMENT_LIBRARY_PATIENT_ID).then((library) => {
      if (!active) return;
      setCareReports((current) => [...current, ...library.reports.filter((report) => !current.some((item) => item.id === report.id))]);
      setDocumentText((current) => [...current, ...library.documents.filter((document) => !current.some((item) => item.patientId === document.patientId && item.reportId === document.reportId))]);
      setCareEvents((current) => addCareEvent(current, { id: 'meera-visit-2026-09-25', patientId: CARE_DOCUMENT_LIBRARY_PATIENT_ID, kind: 'Follow-up', title: 'Clinic visit', date: '2026-09-25', time: '11:00', repeatUntil: '', instructions: 'Bring previous reports and the current prescription.', addedBy: 'Kavya Raghavan' }));
      setLibrarySeeded(CARE_DOCUMENT_LIBRARY_VERSION);
    }).catch(() => { /* Document uploads remain available if the starter files cannot be prepared. */ });
    return () => { active = false; };
  }, [librarySave, reportSave, documentSave, eventSave, taskSave, librarySeeded, setCareReports, setDocumentText, setCareEvents, setLibrarySeeded]);
  ...
  function canReadPortalView(nextView: string, patientId = portalPatientId) {
    return !isFamilySession || canFamilyOpenView(careCircle, patientId, portalAuthor, nextView);
  }
  const canReadCareNote = canReadPortalView('my-plan');
  const canReadCalendar = canReadPortalView('calendar');
  const canReadReports = canReadPortalView('reports');
  ...
  function recordedStoryFor(id: string): CareStoryEntry[] {
    ...
    return [
      ...appointments.filter((visit) => canReadPortalView('calendar', id) && visit.hospitalId === id && visit.status === 'Attended').map((visit): CareStoryEntry => ({ ...common, id: `record-visit-${visit.id}`, date: visit.date, kind: 'Visit', title: visit.type, details: visit.outcome ?? visit.note ?? '', source: 'Appointment record', sourceAuthor: visit.clinician, createdBy: visit.clinician, updatedBy: visit.clinician })),
      ...(recordStates[id]?.releases ?? []).filter((release) => canReadPortalView('my-plan', id) && release.patientId === id).map((release): CareStoryEntry => ({ ...common, id: `record-note-${id}-${release.number}`, date: release.releasedAt.slice(0, 10), kind: 'Milestone', title: `Care note approved · Version ${release.number}`, details: release.fields?.topics ?? release.source ?? '', source: `Approved care note V${release.number}`, sourceAuthor: release.physician, createdBy: release.physician, updatedBy: release.physician })),
      ...careReports.filter((report) => canReadPortalView('reports', id) && report.patientId === id).map((report): CareStoryEntry => ({ ...common, id: `record-report-${report.id}`, date: report.date, kind: 'Report', title: report.file.name, details: '', source: 'Uploaded report', sourceAuthor: report.addedBy, createdBy: report.addedBy, updatedBy: report.addedBy })),
    ];
  }
  ...
  function treatmentEventsFor(id: string): LabTreatmentEvent[] {
    return [
      ...careStory.filter((entry) => canReadPortalView('my-plan', id) && entry.patientId === id && (entry.kind === 'Treatment' || entry.kind === 'Treatment change')).map((entry): LabTreatmentEvent => ({ id: entry.id, patientId: id, date: entry.date, title: entry.title, status: entry.date > careToday ? 'planned' : 'recorded', sourceName: entry.source || `Care timeline · ${entry.createdBy}`, reportId: entry.documentSource?.reportId })),
      ...careEvents.filter((entry) => canReadPortalView('calendar', id) && entry.patientId === id && ['Chemotherapy', 'Radiotherapy', 'Procedure'].includes(entry.kind)).map((entry): LabTreatmentEvent => ({ id: entry.id, patientId: id, date: entry.date, title: entry.title, status: careChecks.some((check) => check.patientId === id && check.eventId === entry.id && check.date === entry.date) ? 'recorded' : 'planned', sourceName: `Treatment calendar · ${entry.addedBy}` })),
    ];
  }

  function saveDocumentTreatment(confirmed: ConfirmedDocumentTreatment) {
    if (!canReadCareNote || !canReadReports || !careReports.some((report) => report.patientId === portalPatientId && report.id === confirmed.source.reportId)) return;
    const entryId = `document-treatment-${crypto.randomUUID()}`;
    setCareStory((current) => {
      if (current.some((entry) => entry.patientId === portalPatientId && entry.date === confirmed.date && entry.title === confirmed.title
        && entry.documentSource?.reportId === confirmed.source.reportId && entry.documentSource.quote === confirmed.source.quote)) return current;
      const next = addCareStoryEntry(current, portalPatientId, portalAuthor, entryId, {
        date: confirmed.date, kind: 'Treatment', title: confirmed.title, details: '', changeReason: '',
        source: confirmed.source.fileName.slice(0, 240), sourceAuthor: '',
      });
      return next.map((entry) => entry.id === entryId ? { ...entry, documentSource: confirmed.source } : entry);
    });
  }
  ...
  function saveDoctorConversation(entry: DoctorConversationEntry, draft?: AssistedNote) {
    ...
    const activeRequest = conversationRequests.find((request) => request.id === activeConversationRequests[entry.patientId] && request.patientId === entry.patientId);
    let episodeId = activeRequest ? careConversationRequestEpisodeId(activeRequest.id) : activeImpactEpisodes[entry.patientId];
    if (!episodeId) {
      episodeId = `doctor:${entry.id}`;
      recordImpactStage({ id: episodeId, patientId: entry.patientId, stage: 'started', at: entry.recordedAt, initiatedBy: 'doctor' });
      setActiveImpactEpisodes((current) => ({ ...current, [entry.patientId]: episodeId! }));
    }
    recordImpactStage({
      id: episodeId,
      ...
      stage: 'completed',
      at: entry.recordedAt,
      initiatedBy: activeRequest?.role,
      requestId: activeRequest?.id,
    });
    if (entry.note.trim()) {
      recordImpactStage({ id: episodeId, patientId: entry.patientId, stage: 'documented', at: entry.recordedAt, initiatedBy: activeRequest?.role, requestId: activeRequest?.id });
      updateSelectedRecord((current) => {
        ...
        if (draft) {
          ...
          const statuses = Object.fromEntries(keys.map((key) => [key, fields[key]?.trim() ? 'ready' : 'not-stated'])) as Record<DraftFieldKey, DraftFieldStatus>;
          for (const key of ['priorities', 'openQuestions', 'followUp'] as DraftFieldKey[]) {
            if (!fields[key]?.trim() && next.draftStatuses[key] === 'clarify' && next.draftPriorVersion !== undefined) {
              fields[key] = next.draftFields[key]; excerpts[key] = next.draftExcerpts[key]; statuses[key] = 'clarify';
            }
          }
          next = reviseSummary(next, { draftFields: fields });
          next = reviseSummary(next, { draftStatuses: statuses, draftExcerpts: excerpts });
        }
        return next;
      });
  ...
  function recordImpactStage(input: {
    id: string;
    patientId: string;
    ...
    setConversationImpactEpisodes((current) => recordCareConversationImpactEvent(current, input));
  }

  function startDoctorImpactEpisode(patientId: string): string {
    const id = `doctor:${crypto.randomUUID()}`;
    const at = new Date().toISOString();
    recordImpactStage({ id, patientId, stage: 'started', at, initiatedBy: 'doctor' });
    setActiveImpactEpisodes((current) => ({ ...current, [patientId]: id }));
    return id;
  }
  ...
  function startConversation() {
    ...
    if (next !== recordState) {
      if (!activeConversationRequests[selectedId] && !activeImpactEpisodes[selectedId]) startDoctorImpactEpisode(selectedId);
  ...
  function renderAccessRequired() {
    return <section className="simple-care-note"><h1>{hindi ? 'अनुमति चाहिए' : 'Ask for access'}</h1><p>{hindi ? 'मरीज़ तय करते हैं कि परिवार के साथ क्या साझा करना है।' : `${portalPatientName.split(' ')[0]} chooses what to share with the family.`}</p><button type="button" className="primary-button" onClick={() => navigate('care-circle')}>{hindi ? 'मेरे अपने' : 'Open care circle'}</button></section>;
  }

  function startConversationRequest(requestId: string) {
    if (!isTreatingPhysician || sessionType !== 'care-team') return;
    const request = conversationRequests.find((entry) => entry.id === requestId && ['requested', 'in-progress'].includes(entry.status));
    if (!request) return;
    const resume = request.status === 'in-progress' && activeConversationRequests[request.patientId] === request.id
      && !recordStates[request.patientId]?.versionPublished && recordStates[request.patientId]?.draftIsNewConversation;
    const episodeId = careConversationRequestEpisodeId(request.id);
    recordImpactStage({ id: episodeId, patientId: request.patientId, stage: 'requested', at: request.createdAt, initiatedBy: request.role, requestId: request.id });
    recordImpactStage({ id: episodeId, patientId: request.patientId, stage: 'started', at: new Date().toISOString(), initiatedBy: request.role, requestId: request.id });
    setConversationRequests((entries) => entries.map((entry) => entry.id === request.id ? { ...entry, status: 'in-progress', updatedAt: new Date().toISOString() } : entry));
    setActiveConversationRequests((links) => ({ ...links, [request.patientId]: request.id }));
    setActiveImpactEpisodes((links) => ({ ...links, [request.patientId]: episodeId }));
    selectPatient(request.patientId);
    setNewDoctorConversation(!resume);
    setView(resume ? 'doctor-review' : 'doctor-record');
  }

  function openConversationVersion(patientId: string, version: number) {
    if (!canReadPortalView('my-plan', patientId)) return;
    if (sessionType === 'care-team') selectPatient(patientId);
    else if (patientId !== portalPatientId) return;
    setNoteSelection({ patientId, version });
    navigate(sessionType === 'care-team' ? 'doctor-history' : 'my-plan');
  }

  function taskPeopleFor(patientId: string): CareTaskAssignee[] {
    return [
      { id: `patient:${patientId}`, name: workItems.find((item) => item.hospitalId === patientId)?.patient ?? portalPatientName, role: 'patient' },
      ...careCircle.members.filter((member) => member.patientId === patientId && member.status === 'active' && member.permissions.calendar).map((member) => ({ id: member.id, name: member.name, role: 'family' as const })),
      { id: `doctor:${patientId}`, name: 'Dr Sujay', role: 'doctor' },
    ];
  }

  function taskViewerFor(patientId: string): CareTaskAssignee {
    if (sessionType === 'care-team') return { id: `doctor:${patientId}`, name: currentRole.split(' · ')[0], role: 'doctor' };
    if (isFamilySession) return { id: activeCareCircleMember(careCircle, patientId, portalAuthor)?.id ?? `family:${patientId}:${portalAuthor}`, name: portalAuthor, role: 'family' };
    return { id: `patient:${patientId}`, name: portalAuthor, role: 'patient' };
  }

  function renderCurrentView() {
    if (!canReadPortalView(view)) return renderAccessRequired();
    switch (view) {
      case 'care-circle':
        return <CareCircle patientId={portalPatientId} patientName={portalPatientName} author={portalAuthor} role={isPatientSession ? 'patient' : isFamilySession ? 'family' : 'doctor'} hindi={hindi} state={careCircle} onChange={setCareCircle} />;
      case 'next-visit':
        return <CareNextVisit patientId={portalPatientId} patientName={portalPatientName} author={portalAuthor} today={careToday} hindi={hindi}
          viewer={taskViewerFor(portalPatientId)} assignees={taskPeopleFor(portalPatientId)}
          appointments={appointments} events={careEvents} tasks={familyTasks} reports={canReadReports ? careReports : []}
          onAddTask={(task) => setFamilyTasks((current) => task.patientId === portalPatientId ? addFamilyTask(current, task) : current)}
          onEditTask={(task) => setFamilyTasks((current) => editFamilyTask(current, portalPatientId, task))}
          onCompleteTask={(taskId, complete) => setFamilyTasks((current) => setFamilyTaskCompleted(current, portalPatientId, taskId, portalAuthor, complete))}
          onOpen={navigate} />;
      case 'medicines':
        return <CareMedicines patientId={portalPatientId} author={portalAuthor} today={careToday} hindi={hindi} events={careEvents} checks={careChecks} reports={careReports} documents={documentText}
          ...
          onUpdate={(event) => {
            ...
            if (updated === careEvents) return;
            setCareEvents(updated); setCareChecks((current) => careChecksAfterEventUpdate(current, previous, event));
          }}
          onCheck={(eventId, date, complete) => setCareChecks((current) => setCareCheck(current, careEvents, portalPatientId, eventId, date, portalAuthor, new Date().toISOString(), careToday, complete))}
          onRemove={(eventId) => { setCareEvents((current) => current.filter((event) => event.patientId !== portalPatientId || event.id !== eventId)); setCareChecks((current) => current.filter((check) => check.patientId !== portalPatientId || check.eventId !== eventId)); }} />;
      ...
      case 'my-timeline': {
        ...
        const entries: TimelineEntry[] = buildHistoryEvents(portalPatientId)
          .filter((entry) => entry.kind !== 'appointment' || canReadCalendar)
          .slice()
      ...
      case 'care-near-me':
        return <CareNearMe audience={isPatientSession ? 'patient' : 'family'} patientName={portalPatientName} homeAddress={isFamilySession ? undefined : detailsFor(portalPatientId).address} />;
      ...
      case 'request-conversation': {
        const request = conversationRequests.filter((entry) => entry.patientId === portalPatientId && entry.requestedBy === portalAuthor && entry.role === sessionType)
          .sort((a, b) => b.createdAt.localeCompare(a.createdAt))[0];
        return <CareConversationRequestForm key={`${portalPatientId}:${portalAuthor}:${request?.id ?? 'new'}`} patientId={portalPatientId} patientName={portalPatientName}
          requester={portalAuthor} requesterRole={isFamilySession ? 'family' : 'patient'} request={request} hindi={hindi}
          onSave={(next) => {
            if (next.patientId !== portalPatientId || next.requestedBy !== portalAuthor || next.role !== sessionType) return;
            setConversationRequests((entries) => entries.some((entry) => entry.id === next.id) ? entries.map((entry) => entry.id === next.id ? next : entry) : [...entries, next]);
            const impactId = careConversationRequestEpisodeId(next.id);
            recordImpactStage({ id: impactId, patientId: next.patientId, stage: 'requested', at: next.createdAt, initiatedBy: next.role, requestId: next.id });
            if (next.status === 'cancelled') recordImpactStage({ id: impactId, patientId: next.patientId, stage: 'cancelled', at: next.updatedAt, initiatedBy: next.role, requestId: next.id });
          }}
          onCancel={() => { if (request) {
            const cancelledAt = new Date().toISOString();
            setConversationRequests((entries) => entries.map((entry) => entry.id === request.id ? { ...entry, status: 'cancelled', updatedAt: cancelledAt } : entry));
            recordImpactStage({ id: careConversationRequestEpisodeId(request.id), patientId: request.patientId, stage: 'cancelled', at: cancelledAt, initiatedBy: request.role, requestId: request.id });
          } }}
          onOpenSignedVersion={(version) => openConversationVersion(portalPatientId, version)} />;
      }
      case 'medicine-access': {
        const key = `${portalPatientId}:${sessionType}:${portalAuthor}`;
        return <CareMedicineAccess hindi={hindi} entry={medicineAccess[key] ?? { location: '', contactName: '', contactPhone: '', enquiry: '', status: 'to-call', updatedAt: '' }}
          onChange={(entry) => setMedicineAccess((current) => ({ ...current, [key]: entry }))}
          careTeam={canReadCareNote ? detailsFor(portalPatientId).localPhysician : undefined} />;
      }
      case 'reports':
      case 'document-search':
        return <CareDocumentSearch key={portalPatientId} patientId={portalPatientId} author={portalAuthor} hindi={hindi} reports={careReports} documents={documentText} onChange={setDocumentText} onReportsChange={setCareReports} editReportId={editReportId} onEditClose={() => setEditReportId(null)} onAddTreatment={canReadCareNote ? saveDocumentTreatment : undefined} onOpenCareStory={() => navigate('care-story')} />;
      ...
      case 'daily-care': {
        const nextVisit = [...appointments.filter((item) => item.hospitalId === portalPatientId && item.status === 'Scheduled' && item.date >= careToday), ...careEvents.filter((item) => item.patientId === portalPatientId && item.kind !== 'Medicine' && item.date >= careToday).map((item) => ({ date: item.date, time: item.time, clinician: item.title }))]
          .sort((a, b) => `${a.date} ${a.time}`.localeCompare(`${b.date} ${b.time}`))[0];
        const signedHomeNote = [...(recordStates[portalPatientId]?.releases ?? [])]
          .filter((release) => release.patientId === portalPatientId && hasTypedCareNoteDoctorSignature(release))
          .sort((first, second) => second.number - first.number)[0];
        return <FamilyCareHome patientName={portalPatientName} hindi={hindi}
          ...
          onMedicineTaken={(eventId, date, complete) => {
            if (!canReadCalendar || !canReadReports) return;
            setCareChecks((current) => setCareCheck(current, careEvents, portalPatientId, eventId, date, portalAuthor, new Date().toISOString(), careToday, complete));
          }} />;
      }
      case 'comfort': {
      ...
      case 'family-tasks':
        return <CareTaskInbox key={`${portalPatientId}:${portalAuthor}`} patientId={portalPatientId} patientName={portalPatientName}
          viewer={taskViewerFor(portalPatientId)} assignees={taskPeopleFor(portalPatientId)} tasks={familyTasks} appointments={appointments} today={careToday} hindi={hindi}
          onAdd={(task) => setFamilyTasks((current) => task.patientId === portalPatientId ? addFamilyTask(current, task) : current)}
          onEdit={(task) => setFamilyTasks((current) => editFamilyTask(current, portalPatientId, task))}
          onComplete={(taskId, complete) => setFamilyTasks((current) => setFamilyTaskCompleted(current, portalPatientId, taskId, portalAuthor, complete))} />;
      case 'visit-questions': {
        ...
        return <FamilyCareWorkspace key={portalPatientId} patientId={portalPatientId} patientName={portalPatientName} author={author}
          ...
          onAdd={(task) => setFamilyTasks((current) => task.patientId === portalPatientId ? addFamilyTask(current, task) : current)}
          onEdit={(task) => setFamilyTasks((current) => editFamilyTask(current, portalPatientId, task))}
          onRemove={(taskId) => setFamilyTasks((current) => removeFamilyTask(current, portalPatientId, taskId))}
          onRestore={(task) => setFamilyTasks((current) => restoreFamilyTask(current, portalPatientId, task))}
      ...
      case 'calendar': {
        ...
        return <>{view === 'calendar' && <div className="family-workspace-heading"><h1>{hindi ? 'कैलेंडर' : 'Calendar'}</h1></div>}
          <CareCalendar patientId={portalPatientId} author={author} today={careToday} hindi={hindi}
            onAppointmentInstructions={saveAppointmentInstructions} onTestResults={() => navigate('lab-history')} onMedicines={() => navigate('medicines')}
            appointments={appointments} tasks={familyTasks} events={careEvents} checks={careChecks} reports={canReadReports ? careReports : []} reportSave={reportSave}
            documentText={canReadReports ? documentText : []} onReadReport={canReadReports ? (id) => { setEditReportId(id); navigate('document-search'); } : undefined}
            onAdd={(event) => setCareEvents((current) => event.patientId === portalPatientId ? addCareEvent(current, event) : current)}
            onUpdate={(event) => {
              const previous = careEvents.find((item) => item.patientId === portalPatientId && item.id === event.id);
              const updated = updateCareEvent(careEvents, portalPatientId, event);
            ...
            onReports={(reports) => { if (canReadReports) setCareReports((current) => [...current, ...reports.filter((report) => report.patientId === portalPatientId && !reportFileError(report.file) && !current.some((existing) => existing.id === report.id))]); }}
      ...
      case 'lab-history': {
        ...
        return <FamilyCareTools tool={view} patientId={portalPatientId} patientName={portalPatientName} author={author} today={careToday} hindi={hindi}
          ...
          symptoms={canReadCareNote ? symptoms : []} story={canReadCareNote ? careStory : []} contacts={handoverContacts} onSymptoms={setSymptoms} onStory={setCareStory} onContacts={setHandoverContacts}
          homeHelp={homeHelp} supportPlaces={supportPlaces} onHomeHelp={setHomeHelp} onSupportPlaces={setSupportPlaces}
          questions={openQuestions} copies={careCopies} releases={canReadCareNote ? recordStates[portalPatientId]?.releases ?? [] : []} onQuestions={setOpenQuestions} onCopies={setCareCopies}
          voiceJournal={voiceJournal.filter((entry) => entry.patientId !== portalPatientId || entry.createdBy === author)} onVoiceJournal={(next) => setVoiceJournal((current) => [...current.filter((entry) => entry.patientId === portalPatientId && entry.createdBy !== author), ...next])} voiceSave={voiceSave}
          costs={familyCosts} onCosts={setFamilyCosts}
          labResults={canReadReports ? labResults : []} onLabResults={setLabResults} treatmentEvents={treatmentEventsFor(portalPatientId)}
          acknowledgements={canReadCareNote ? noteAcknowledgements : []} signedRelease={canReadCareNote && hasTypedCareNoteDoctorSignature(release) ? release : undefined}
          ...
          reports={canReadReports ? careReports : []} onOpen={navigate} onCareNote={() => navigate('my-plan')} onReports={() => navigate('reports')}
          onReadReport={canReadReports ? (reportId) => { setEditReportId(reportId); navigate('document-search'); } : undefined}
          note={canReadCareNote && hasTypedCareNoteDoctorSignature(release) ? { version: release.number, physician: release!.signature!.name, releasedAt: release!.releasedAt, fields: planFieldsFor(portalPatientId) } : null}
          calendarText={canReadCalendar ? [
          ...
          reportNames={canReadReports ? careReports.filter((report) => report.patientId === portalPatientId).map((report) => `${report.date} — ${report.file.name}`) : []} />;
      }
      case 'my-doctors': {
        const patientAppointments = appointments.filter(
          (a) => canReadCalendar && a.hospitalId === portalPatientId && a.status !== 'Scheduled',
        );
  ...
  if (simpleMode && authScreen === null) {
    const saves = [preparationSave, taskSave, eventSave, checkSave, reportSave, appointmentSave, noteSave, symptomSave, storySave, contactSave, helpSave, placesSave, questionSave, copySave, comfortSave, voiceSave, costSave, labSave, conversationSave, profileSave, documentSave, acknowledgementSave, librarySave, circleSave, requestSave, requestLinkSave, medicineAccessSave];
    ...
    return <SimpleCareShell view={view} role={role} hindi={hindi} saveStatus={saveStatus}
      ...
      {!canReadPortalView(view) ? renderAccessRequired()
        ...
        conversationRequests={conversationRequests} conversationImpactEpisodes={conversationImpactEpisodes} conversationImpactLoading={impactSave === 'loading' || requestSave === 'loading'} onStartConversationRequest={startConversationRequest} onOpenConversationVersion={openConversationVersion}
        onReview={(id) => { selectPatient(id); setView('doctor-review'); }}
        onRecord={(id) => { const patientId = id ?? selectedId; if (id) selectPatient(id); startDoctorImpactEpisode(patientId); setNewDoctorConversation(true); setView('doctor-record'); }}
        onHistory={(id) => { if (id) selectPatient(id); setView('doctor-history'); }} onInstructions={saveAppointmentInstructions}
        onTasks={(id) => { selectPatient(id); setView('family-tasks'); }} />
        : role === 'doctor' && view === 'doctor-record' ? <DoctorConversation key={`${selectedId}:${newDoctorConversation}`}
          ...
          referenceNote={currentRelease?.source || recordState.draftSource}
          previousContext={currentRelease?.signature ? { version: currentRelease.number, releasedAt: currentRelease.releasedAt, fields: currentRelease.fields } : undefined}
          requestContext={conversationRequests.find((request) => request.patientId === selectedId && request.id === activeConversationRequests[selectedId] && request.status === 'in-progress')}
        ...
        : role === 'doctor' && view === 'doctor-review' ? <DoctorNoteReview key={selectedId}
          patientId={selectedId} patientName={selectedItem.patient} patients={workItems.map((item) => ({ id: item.hospitalId, name: item.patient }))}
          state={recordState} physician={currentRole.split(' · ')[0]} canSign={isTreatingPhysician && sessionType === 'care-team'} onPatient={selectPatient}
          ...
          onFields={(fields) => {
            ...
            updateSelectedRecord((current) => {
              ...
              const changed = keys.filter((key) => fields[key] !== current.draftFields[key]);
              if (!changed.length) return current;
              const nextFields = { ...current.draftFields }; const statuses = { ...current.draftStatuses }; const excerpts = { ...current.draftExcerpts };
              for (const key of changed) {
                const value = fields[key];
                const trimmed = value.trim();
                nextFields[key] = trimmed ? value : 'Not stated in this conversation.';
                statuses[key] = !trimmed || trimmed === 'Not stated in this conversation.' ? 'not-stated' : 'ready';
                excerpts[key] = statuses[key] === 'ready' ? current.draftSource : '';
              }
              const next = reviseSummary(current, { draftFields: nextFields, draftExcerpts: excerpts });
              return reviseSummary(next, { draftStatuses: statuses });
            });
          }}
          onConfirmField={(key) => {
            if (!isTreatingPhysician || sessionType !== 'care-team') return;
            updateSelectedRecord((current) => current.draftExcerpts[key]?.trim() && current.draftSource.includes(current.draftExcerpts[key])
              ? setSummaryFieldStatus(current, key, 'ready') : current);
          }}
          onApprove={(permission, reviewed, signature, noteKind) => {
            ...
            const release = latestSummaryRelease(next)!;
            updateSelectedRecord(() => ({ ...next, retrievalAcknowledged: false, retrievalUnlocked: false, unlockedVersion: null }));
            const requestId = activeConversationRequests[selectedId];
            const impactEpisodeId = requestId ? careConversationRequestEpisodeId(requestId) : activeImpactEpisodes[selectedId];
            if (impactEpisodeId) recordImpactStage({ id: impactEpisodeId, patientId: selectedId, stage: 'signed', at: release.releasedAt, signedVersion: release.number });
            if (requestId) {
              setConversationRequests((entries) => entries.map((entry) => entry.id === requestId && entry.patientId === selectedId && entry.status === 'in-progress'
                ? { ...entry, status: 'completed', signedVersion: release.number, updatedAt: release.releasedAt } : entry));
              setActiveConversationRequests((links) => { const nextLinks = { ...links }; delete nextLinks[selectedId]; return nextLinks; });
            }
            if (impactEpisodeId) setActiveImpactEpisodes((links) => { const nextLinks = { ...links }; delete nextLinks[selectedId]; return nextLinks; });
            addAudit(`Signed care note version ${release.number}`, selectedItem.patient, 'PUBLISH', undefined, selectedId, release.number);
          }}
          onFinishLater={() => navigate('home')} onFamily={(version) => { if (version) openConversationVersion(selectedId, version); }} />
        : view === 'my-plan' || view === 'note-history' || view === 'doctor-history' ? <CareNoteReader key={`${portalPatientId}:${noteSelection?.patientId === portalPatientId ? noteSelection.version : 'latest'}`}
          patientId={portalPatientId} patientName={portalPatientName} releases={recordStates[portalPatientId]?.releases ?? []} hindi={hindi}
          initialVersion={noteSelection?.patientId === portalPatientId ? noteSelection.version : undefined}
